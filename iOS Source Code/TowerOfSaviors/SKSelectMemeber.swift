...
/// 编辑队伍时, 从其中选择成员
class SKSelectMemeber: SKScene, SceneClickableType, SKGridViewDelegate {
    ...
    /// 右上角的排序按钮
    private lazy var sortTitle = childNode(withName: "标题栏/右按钮/title") as! SKLabelNode
    ...
    func scene(_ scene: SKScene, click button: SKButton) {
        ...
        if button.name == "monster" {
            ...
            let monster = monsters[cell.indexPath! - 1]
            
            let userData = NSMutableDictionary()
            if let dictionary = scene.userData as? [AnyHashable : Any] {
                userData.addEntries(from: dictionary)
            }
            userData["monster"] = monster
    ...
    private func reloadCards() {
        ...
        switch sortType {
        case "编号":
            monsters = filtered.sorted { return $0.怪兽标识 < $1.怪兽标识 }
        case "属性":
            monsters = filtered.sorted { return $0.元素属性.rawValue < $1.元素属性.rawValue }
        case  "生命":
            monsters = filtered.sorted { return $0.满级生命 > $1.满级生命 }
        case  "攻击":
            monsters = filtered.sorted { return $0.满级攻击 > $1.满级攻击 }
        case  "回复":
            monsters = filtered.sorted { return $0.满级回复 > $1.满级回复 }
        case  "空间":
            monsters = filtered.sorted { return $0.占据空间 < $1.占据空间 }
        case  "系列":
            monsters = filtered.sorted { return $0.所属系列 < $1.所属系列 }
        default:
            monsters = filtered
        }
    ...
    func gridView(_ gridView: SKGridView, cellForItemAt indexPath: Int) -> SKGridViewCell {
        ...
        if indexPath == 0 {
            ...
            return collection.dequeueReusableCell(withIdentifier: SKPackageRemoveCell.defaultIdentifier, for: indexPath) as! SKPackageRemoveCell
            
        } else {
            
            let monster = monsters[indexPath - 1]
            ...
            case "生命":
                cell.labelNode.text = "\(monster.满级生命)"
            case "攻击":
                cell.labelNode.text = "\(monster.满级攻击)"
            case "回复":
                cell.labelNode.text = "\(monster.满级回复)"
            case "空间":
                cell.labelNode.text = "\(monster.占据空间)"
            default:
                cell.labelNode.text = "No.\(monster.怪兽标识)"
            }
